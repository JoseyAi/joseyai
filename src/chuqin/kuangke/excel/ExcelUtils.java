...
public class ExcelUtils {
	...
	public static void initExcel(String fileName, String[] colName) {
		...
		try {
			...
			WritableSheet sheet = workbook.createSheet("出勤旷课表", 0);
	...
	public static <T> void writeObjListToExcel(List<T> objList,
		...
		if (objList != null && objList.size() > 0) {
			...
			try {
				...
				writebook.write();
				Toast.makeText(c, "导出到手机存储中文件夹kuangke成功", Toast.LENGTH_SHORT).show();
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				if (writebook != null) {
	...
	public static  List<KkObject> read2DB(File f, Context con) {
		ArrayList<KkObject> chdList = new ArrayList<KkObject>();
		try {
