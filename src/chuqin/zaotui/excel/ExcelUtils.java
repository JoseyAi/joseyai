...
public class ExcelUtils {
	...
	public static void initExcel(String fileName, String[] colName) {
		...
		try {
			...
			WritableSheet sheet = workbook.createSheet("出勤早退表", 0);
	...
	public static <T> void writeObjListToExcel(List<T> objList,
		...
		if (objList != null && objList.size() > 0) {
			...
			try {
				...
				writebook.write();
				Toast.makeText(c, "导出到手机存储中文件夹zaotui成功", Toast.LENGTH_SHORT).show();
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				if (writebook != null) {
	...
	public static  List<ZtObject> read2DB(File f, Context con) {
		ArrayList<ZtObject> zaotuiList = new ArrayList<ZtObject>();
		try {
			...
			for (int i = 1; i < sheet.getRows(); i++) {
				ZtObject tc = new ZtObject();
				cell = sheet.getCell(1, i);
				tc.setName(cell.getContents());
				cell = sheet.getCell(2, i);
				tc.setNumber(cell.getContents());
				cell = sheet.getCell(3, i);
				tc.setXibie(cell.getContents());
				cell = sheet.getCell(4, i);
				tc.setBanji(cell.getContents());
				cell = sheet.getCell(5, i);
				tc.setCourse(cell.getContents());
				cell = sheet.getCell(6, i);
				tc.setReason(cell.getContents());
				cell = sheet.getCell(7, i);
				tc.setQuxiang(cell.getContents());
				Log.d("gaolei", "Row"+i+"---------"+tc.getName() + tc.getNumber()
						+ tc.getXibie() + tc.getBanji() + tc.getCourse()
						+ tc.getReason() + tc.getQuxiang());
				zaotuiList.add(tc);
