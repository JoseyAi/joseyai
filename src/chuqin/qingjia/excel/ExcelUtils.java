...
public class ExcelUtils {
	...
	public static void initExcel(String fileName, String[] colName) {
		...
		try {
			...
			WritableSheet sheet = workbook.createSheet("出勤请假表", 0);
	...
	public static <T> void writeObjListToExcel(List<T> objList,
		...
		if (objList != null && objList.size() > 0) {
			...
			try {
				...
				writebook.write();
				Toast.makeText(c, "导出到手机存储中文件夹qingjia成功", Toast.LENGTH_SHORT).show();
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				if (writebook != null) {
	...
	public static  List<QjObject> read2DB(File f, Context con) {
		ArrayList<QjObject> chdList = new ArrayList<QjObject>();
		try {
			...
			for (int i = 1; i < sheet.getRows(); i++) {
				...
				tc.setFxtime(cell.getContents());
				Log.d("gaolei", "Row"+i+"---------"+tc.getName() + tc.getNumber()
						+ tc.getXibie() + tc.getBanji() + tc.getReason()
						+ tc.getTime() + tc.getFxtime());
				chdList.add(tc);
