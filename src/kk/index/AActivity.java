...
import kebiao.editschedule.SetActivity;
import kebiao.temp.DataBase;
import kebiao.temp.MyApplication;
import kebiao.temp.MyDialog;
import kebiao.temp.ShareMethod;
...
public class AActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.kebiao_main);		
		//将该activity加入到MyApplication对象实例容器中
		MyApplication.getInstance().addActivity(this);
		
		db=new DataBase(AActivity.this);
...
//			finish();			
		...
		//用适配器为各选项卡添加所要显示的内容
		for(int i=0;i<7;i++){
			cursor[i]=AActivity.db.select(i);		
		...
		setButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent intent = new Intent(AActivity.this, SetActivity.class);
		...
		for( int day=0;day<7;day++){
			...
			list[day].setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View arg1,
					...
					final int n=id;
				    final AlertDialog.Builder builder = new AlertDialog.Builder(AActivity.this);
					...
					if((tv.getText()).toString().equals("")){
						...
						builder.setItems(R.array.edit_options1, new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {					
								...
								if(which == 0){					
									new MyDialog(AActivity.this).add(currentDay,n);
								}
							}
						});
						builder.create().show();
					...
					//否则启动修改对话框，或直接删除数据
						...
						builder.setItems(R.array.edit_options2, new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								//如果单击了该列表项，则跳转到编辑课程信息的界面
								if(which == 0){					
									new MyDialog(AActivity.this).modify(currentDay,n);
								}
								if(which == 1){
									...
									switch(n2){
										case 0:
											for(int m=0;m<n1;m++){
												AActivity.db.deleteData(currentDay,n+m+1);
												}
											break;
	
										case 1:
											AActivity.db.deleteData(currentDay,n);
											for(int m=1;m<n1;m++){
												AActivity.db.deleteData(currentDay,n+m);
												}
											break;		
										case 2:
											AActivity.db.deleteData(currentDay,n-1);
											AActivity.db.deleteData(currentDay,n);
											for(int m=2;m<n1;m++){
												AActivity.db.deleteData(currentDay,n+m-1);
												}
	...
	public SimpleCursorAdapter adapter(int i){
		return new SimpleCursorAdapter(this, R.layout.kebiao_list_v2,cursor[i],new String[]{"_id","classes","location",
